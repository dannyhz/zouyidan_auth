...
/**
 * 该类是一个使用{@link ReferenceType#SOFT soft} 或 {@linkplain ReferenceType#WEAK weak}
 * （引用）keys和values的{@link ConcurrentHashMap}
 *
 * <p>该类可用于替代{@code Collections.synchronizedMap(new WeakHashMap<K, Reference<V>>())} 的
 * 使用场景，以便于在同步操作时获得更好的性能。 该实现同 {@link ConcurrentHashMap}一样，支持{@code null}的keys和values.
 *
 * <p><b>注意:</b> 使用references意味着不需要保证放入map中的items一直有效。gc可能在任何时候找到这些references，以至于
 * entries会被默默的回收掉。
 *
 * <p>默认使用{@linkplain SoftReference soft entry references}
 *
 * @author Phillip Webb
 * @since 3.2
 */
