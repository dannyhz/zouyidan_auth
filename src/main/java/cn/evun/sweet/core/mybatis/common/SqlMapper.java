...
/**
 * MyBatis执行sql工具
 * @link http://git.oschina.net/free/Mapper/tree/Mapper2.x
 * 
 * @author liuzh
 */
public class SqlMapper {
	 ...
	 * 将源Map中的key进行驼峰转换后输出
	 */
	public Map<String, Object> mapKeyUnderscoreToCamelCase(Map<String, Object> source){
		if(source == null){
			return null;
		}
		Map<String, Object> result = new HashMap<String, Object>(source.size());
		for(Map.Entry<String, Object> entry : source.entrySet()){
			result.put(StringUtils.underlineToCamelhump(entry.getKey().toLowerCase()), entry.getValue());
		}
		return result;
	}
